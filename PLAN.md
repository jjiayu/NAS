...
9. `config/` — RobotModel/Scenario/PlannerConfig runtime, migration d'`environments.hpp`. **Sous-étapes (2026-09-18) :**
  ...
  - 9d. Import de scène depuis fichier STL (parser + regroupement des triangles coplanaires en surfaces convexes) plutôt que des listes de points codées en dur — feature à part, plus grosse, après 9a-9c. **FAIT (2026-09-18)** : `load_scenario_from_stl(path, robot_model)`, parseur ASCII+binaire, regroupement par plan quantifié (~1e-4 de tolérance sur normale+offset), réutilise `Surface` telle quelle. A révélé un vrai bug latent dans `core/surface` (voir `docs/paper-deltas.md`), corrigé et revalidé contre le golden. Vérifié visuellement via `apps/stl_preview` sur un cube de test : les 12 triangles donnent bien 6 surfaces, chacune correctement rétrécie.
...
10. `apps/` — drivers CLI minces. **FAIT partiellement (2026-09-18)** : `apps/astar_plan` (CASSR uniquement, `astar_grid_plan`/`nas_plan` viendront avec les phases 13/6). Prend un nom de scène (`config::available_scenarios()`), un `PlannerConfig` JSON, un dossier `talosReachability`, et dump le même JSON que `tests/viz_dump/dump_plan.cpp` (surfaces/chemin/pas) — pas de tests unitaires propres (wrapper fin sur du code déjà testé), testé manuellement sur `NarrowPassage`/`ThreePathsNAS` : tailles de chemin exactes (30/20, soit 29/19 pas) et QP réussi sur les deux. Egalement ajouté `apps/stl_preview` (dump surfaces-only d'une scène STL importée, sans recherche/QP — outil de vérification visuelle pour la phase 9d).
11. `viz/` découplée — prototype meshcat-cpp + export minimal pour figures. **`tests/viz_dump/dump_plan.cpp` (2026-09-18) est déjà la graine de la moitié "export statique"** — outil permanent (pas supprimé), généralisé à n'importe quel scénario de `tests/fixtures`, dump JSON (surfaces/chemin/pas) consommé par un rendu SVG à la demande. **Moitié interactive : `viz/meshcat_view.py` (2026-09-19)** — client Python `meshcat-python` (déjà dans l'env) qui affiche le JSON de `astar_plan`/`dump_plan` (surfaces, pieds orientés bleu/orange, ligne de foulée) dans un `meshcat-server` ; validé visuellement par l'utilisateur sur `NarrowPassage`. Le C++ `meshcat-cpp` reste écarté : **Historique de la décision (2026-09-18)**  : `meshcat-cpp` (fork actif `gbionics/meshcat-cpp`, l'original `ami-iit` est archivé sous "backup") embarque tout un serveur websocket (`uWebSockets`+`uSockets`) + `msgpack-cxx` + `stduuid` + `CMakeRC` pour un besoin qui n'est en réalité qu'un client parlant au `meshcat-server` Python déjà installé — chaîne de dépendances jugée trop lourde à vendoriser sur cette machine (RAM déjà tendue même pour des builds CGAL simples). Décision utilisateur : reporter, revenir dessus plus tard (soit vendoriser malgré tout, soit écrire un client maison minimal websocket+msgpack).
12. `bindings/` Python (nanobind) — DTO plat de résultat, GIL relâché pendant le solve, couche 0 seulement. **FAIT (2026-09-18)** : `nas_bindings.plan(scenario_name, planner_config_path, talos_reachability_data_dir) -> FootstepResult{success, positions, stance_feet, foot_yaws}`. `nanobind` installé via `pip install nanobind` dans l'env `rwa` (léger, pas de chaîne de dépendances contrairement à meshcat-cpp — décision utilisateur d'installer). Piège de build résolu : les libs statiques du projet (`nas_config` et toute sa chaîne) ne sont pas compilées en PIC par défaut (prévues pour des exécutables) — un module Python est un `.so`, qui exige du PIC partout ; `CMAKE_POSITION_INDEPENDENT_CODE ON` mis dans `bindings/CMakeLists.txt` avant les `add_subdirectory`. 4 tests Python (`bindings/tests/test_bindings.py`, exécutés via `ctest`), validés sur `NarrowPassage`/`ThreePathsNAS` (mêmes tailles de chemin que le golden).
13. `planners/grid_astar_search` — baseline, priorité basse. **FAIT (2026-09-18)** : `GridEnvironment` + `GridAstarSearch`, réutilisent `ReachabilityModel` (Forward) et `NodePool`. Pas de golden (la phase 0 n'a jamais capturé de référence grille) : 4 tests directs (rasterisation, chemin sur `Flat`, but inatteignable, `NarrowPassage` non traversable à 5 cm — comportement de l'ancien code reproduit, mais **le papier (Table I) montre son A* discrétisé traversant cette scène** : écart ouvert, voir README du module). Deux morceaux de code mort de l'ancien code non portés (voir `docs/paper-deltas.md`). Incident de ressources pendant le portage (allocation de tous les candidats dans le pool → OOM), corrigé par un prédicat `should_skip` avant allocation.
...
- **TODO (2026-09-20, décision utilisateur) : le rétrécissement des surfaces devient un paramètre de la config.** Aujourd'hui chaque surface est rétrécie de la demi-longueur et de la demi-largeur du pied (`RobotModel::foot_length/foot_width` = 0,22 m, soit 11 cm) dans le constructeur de `Surface`, sans que l'utilisateur le choisisse (le papier, VI, dit au contraire qu'on peut se poser au bord). Cible : l'utilisateur définit un seuil `d` en cm, et les surfaces réellement utilisées par le problème (recherche, patchs, contraintes du QP) sont réduites d'autant. Comment le paramétrer (une valeur par surface ? isotrope ou selon x/y ? défaut ?) : à voir plus tard, pas décidé. Voir `docs/paper-corrections-draft.md`, point 5.
- **TODO (2026-09-22, décision utilisateur) : au nettoyage final, purger les dépendances aux bibliothèques QP non retenues.** On compare actuellement quadprog (backend en place, `eiquadprog`) et ProxQP (`proxsuite`, à ajouter) sur les 15 scènes. Une fois le choix de backend tranché, retirer du build/CMake/conda la ou les bibliothèques QP non gardées (et tout backend de test associé), pas seulement le code mort.
