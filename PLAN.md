...
13. `planners/grid_astar_search` — baseline, priorité basse. **FAIT (2026-09-18)** : `GridEnvironment` + `GridAstarSearch`, réutilisent `ReachabilityModel` (Forward) et `NodePool`. Pas de golden (la phase 0 n'a jamais capturé de référence grille) : 4 tests directs (rasterisation, chemin sur `Flat`, but inatteignable, `NarrowPassage` non traversable à 5 cm — limite attendue de la baseline). Deux morceaux de code mort de l'ancien code non portés (voir `docs/paper-deltas.md`). Incident de ressources pendant le portage (allocation de tous les candidats dans le pool → OOM), corrigé par un prédicat `should_skip` avant allocation.
