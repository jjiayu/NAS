...
11. `viz/` découplée — prototype meshcat-cpp + export minimal pour figures. **`tests/viz_dump/dump_plan.cpp` (2026-09-18) est déjà la graine de la moitié "export statique"** — outil permanent (pas supprimé), généralisé à n'importe quel scénario de `tests/fixtures`, dump JSON (surfaces/chemin/pas) consommé par un rendu SVG à la demande. **Moitié meshcat-cpp interactive repoussée (2026-09-18)** : `meshcat-cpp` (fork actif `gbionics/meshcat-cpp`, l'original `ami-iit` est archivé sous "backup") embarque tout un serveur websocket (`uWebSockets`+`uSockets`) + `msgpack-cxx` + `stduuid` + `CMakeRC` pour un besoin qui n'est en réalité qu'un client parlant au `meshcat-server` Python déjà installé — chaîne de dépendances jugée trop lourde à vendoriser sur cette machine (RAM déjà tendue même pour des builds CGAL simples). Décision utilisateur : reporter, revenir dessus plus tard (soit vendoriser malgré tout, soit écrire un client maison minimal websocket+msgpack).
