...
**Étape courante : phases 0, 1, 2, 3, 4 faites — tout Stage A jusqu'au socle commun (`core/*`) est en place. Passage à la phase 5 (port CASSR, `planners/astar_search`).**
**Prochaine action : porter `AstarSearch` sur `core/expansion`/`core/node`/`core/reachability`, comparer contre le golden de la phase 0 (séquences de nœuds).**
