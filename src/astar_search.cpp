...
AstarSearch::AstarSearch() {
  ...
  std::cout << "\n[ Generate Environment Inforamtion (Get Surface Parameters) ]"
            << std::endl;
  ...
  this->goal_location = surfaces.back().centroid +
                        goal_offset; // Add offset vector to centroid point
  std::cout << "  - Node counter: " << this->node_counter << std::endl;
  std::cout << "  - Goal Stance Foot: "
            << (goal_stance_foot == 0   ? "LEFT FOOT (0)"
                : goal_stance_foot == 1 ? "RIGHT FOOT (1)"
                                        : "INVALID")
            << std::endl;
  std::cout << "  - Goal Location (World Frame): " << this->goal_location
            << std::endl;
  ...
  std::cout << "  - Cycle Detection: "
            << (this->cycle_detection_flag ? "ON" : "OFF") << std::endl;

  // Initialize the start node
  Node *start_node = new Node();
  start_node->parent_ptrs = std::vector<Node *>(); // Empty vector for root node
  start_node->node_id = this->node_counter++;
  start_node->patch_vertices = std::vector<Point_3>(
      {current_foot_pos}); // Already Point_3, no conversion needed
  ...
  start_node->h_score =
      compute_euclidean_distance(current_foot_pos, this->goal_location);
...
void AstarSearch::search() {
  ...
  while (!open_set.empty()) {
    ...
    if (current_node->stance_foot == goal_stance_foot &&
      ...
      auto duration = std::chrono::duration_cast<std::chrono::microseconds>(
          end_time - start_time);
      std::cout << "A-star Time taken: " << std::fixed << std::setprecision(3)
                << duration.count() / 1000.0 << " milliseconds (ms)"
                << std::endl;
      std::cout << "Total node expansion count: " << this->expansion_coount
                << std::endl;
      std::cout << "Total minkowski time: " << this->total_minkowski_time
                << " ms" << std::endl;
      std::cout << "Total clipping time: " << this->total_clipping_time << " ms"
                << std::endl;
      std::cout << "Total plane-polytope intersection time: "
                << this->total_plane_polytope_intersect_time << " ms"
                << std::endl;
      std::cout << "Total polygon intersection time: "
                << this->total_polygon_2d_intersect_time << " ms" << std::endl;
      ...
      for (Node *node : this->result_path) {
        std::cout << "Node ID: " << node->node_id
                  ...
                  << ", Stance Foot: " << node->stance_foot;
        if (foot_yaw_rotation_flag) {
          std::cout << ", Foot Yaw: " << std::fixed << std::setprecision(3)
                    << node->foot_yaw << " rad ("
                    << (node->foot_yaw * 180.0 / M_PI) << "°)";
        }
        std::cout << std::endl;
      }
      break;
    ...
    for (Node *child : children) {
      ...
      // double tentative_g_score = current_node->g_score +
      // compute_euclidean_distance(current_node->centroid, child->centroid);
      double tentative_g_score =
          current_node->g_score + 1.0 + 1 * abs(child->foot_yaw);
      double tentative_h_score = 0.0;
      if (this->distance_metric == "gjk") {
        tentative_h_score =
            10.0 * calculate_gjk_distance_point_to_patch(child->patch_vertices,
                                                         this->goal_location);
      } else if (this->distance_metric == "epa") {
        tentative_h_score =
            10.0 * calculate_epa_distance_point_to_patch(child->patch_vertices,
                                                         this->goal_location);
      } else if (this->distance_metric == "euclidean") {
        tentative_h_score =
            compute_euclidean_distance(child->centroid, this->goal_location);
      }
  ...
  auto failure_time = std::chrono::duration_cast<std::chrono::microseconds>(
      std::chrono::high_resolution_clock::now() - start_time);
  std::cout << "If fail, then the computation time is: "
            << failure_time.count() / 1000.0 << " ms" << std::endl;
  std::cout << "\n[ A* search completed ]" << std::endl;
  std::cout << "Total nodes expanded: " << this->node_counter << std::endl;
  std::cout << "Total nodes in closed set: " << this->closed_set.size()
            << std::endl;
  std::cout << "Total nodes in open set: " << this->open_set.size()
            << std::endl;
}
...
std::vector<Node *> AstarSearch::get_children(Node *parent) {
  ...
  // Step 1: Compute minkowski sum based on the patch vertices and the base
  // polytope
  // TODO: make sure we get the correct base polytop

  auto start_time = std::chrono::high_resolution_clock::now();

  Polyhedron base_polytope = parent->stance_foot == 0 ? this->rf_in_lf_polytope
                                                      : this->lf_in_rf_polytope;

  // Rotate the polytope based on the parent's foot yaw angle if foot yaw
  // rotation is enabled
  ...
  for (const auto &surface : surfaces) {
    ...
    std::vector<Point_3> polytope_plane_intersect_pts_3d =
        compute_polytope_plane_intersection(surface.plane, P_union);
    end_time = std::chrono::high_resolution_clock::now();
    duration = std::chrono::duration_cast<std::chrono::microseconds>(
        end_time - start_time);
    this->total_plane_polytope_intersect_time += duration.count() / 1000.0;

    // Sub-Step 2: Compute intersection between polygons (if we have the
    // polytope and the plane has intersection)

    if (polytope_plane_intersect_pts_3d.size() > 2) {
      // Convert 3d intersection points to 2d surface plane
      std::vector<Point_2> polytope_plane_intersect_pts_2d =
          transform_3d_points_to_surface_plane(polytope_plane_intersect_pts_3d,
                                               surface.transform_to_surface);

      // Convert polytope plane intersection points into convex hull
      Polygon_2 polytope_plane_intersect_convex_hull;
      CGAL::convex_hull_2(
          polytope_plane_intersect_pts_2d.begin(),
          polytope_plane_intersect_pts_2d.end(),
          std::back_inserter(polytope_plane_intersect_convex_hull));
      std::vector<Point_2> polytope_plane_intersect_convex_hull_pts;
      for (auto it = polytope_plane_intersect_convex_hull.vertices_begin();
           it != polytope_plane_intersect_convex_hull.vertices_end(); ++it) {
        polytope_plane_intersect_convex_hull_pts.push_back(*it);
      }

      // compute intersection between 2d intersection polygon (subject polygon)
      // and the surface polygon (clipping polygon)
      start_time = std::chrono::high_resolution_clock::now();

      std::vector<Point_2> polygon_2d_intersect_pts =
          compute_2d_polygon_intersection(
              polytope_plane_intersect_convex_hull_pts, surface.vertices_2d);
      ...
      // Sub-Step 3: Convert the 2D intersection polygon to 3D (using the
      // inverse transformation), only if we have polygon intersection result
      //             Also create the child node
      if (polygon_2d_intersect_pts.size() > 2) {

        Polygon_2 polygon_2d_intersect_result;
        CGAL::convex_hull_2(polygon_2d_intersect_pts.begin(),
                            polygon_2d_intersect_pts.end(),
                            std::back_inserter(polygon_2d_intersect_result));
        std::vector<Point_3> polytope_surf_3d_intersect_pts =
            transform_2d_points_to_world(polygon_2d_intersect_pts,
                                         surface.transform_to_3d);
        Polyhedron
            polytope_surf_3d_intersect_polygon; // Create intersection polygon
                                                // (just for visualization)
        CGAL::convex_hull_3(polytope_surf_3d_intersect_pts.begin(),
                            polytope_surf_3d_intersect_pts.end(),
                            polytope_surf_3d_intersect_polygon);

        // // Visualization
        // auto renderWindow = Visualizer::create_figure("3D Polytope-Surface
        // Intersection Visualization"); auto renderer =
        // renderWindow->GetRenderers()->GetFirstRenderer();
        // Visualizer::add_polyhedron(renderer, surface.polyhedron_3d,
        // (double[]){0.7, 0.9, 1.0}, 0.3);  // Add Surface (light blue)
        // Visualizer::add_polyhedron(renderer, P_union, (double[]){1.0, 0.7,
        // 0.8}, 0.5);  // Add P_union (pink)
        // Visualizer::add_polyhedron(renderer,
        // polytope_surf_3d_intersect_polygon, (double[]){0.0, 1.0, 0.0}, 0.7);
        // // Add intersection polygon (green) Visualizer::add_points(renderer,
        // polytope_surf_3d_intersect_pts, (double[]){1.0, 0.0, 0.0}, 0.05);  //
        // Add intersection points (red) Visualizer::show(renderWindow); // Show
        // the 3D visualization

        // Found intersection, create child node
        // Filter children if it has been detected as a cycle
        int stance_foot =
            parent->stance_foot == 0 ? 1 : 0; // Alternate stance foot
        if ((this->cycle_detection_flag == true) &&
            (cycle_path_detection(parent, stance_foot, surface.surface_id) ==
             true)) {
          continue; // the same surface visited 2 steps before
        }

        // Create child nodes with different foot yaw angles if rotation is
        // enabled
        std::vector<double> yaw_angles;
        if (foot_yaw_rotation_flag) {
          // Generate discretized yaw angles relative to parent's foot yaw
          for (int i = -foot_yaw_angle_discretization_num;
               i <= foot_yaw_angle_discretization_num; ++i) {
            yaw_angles.push_back(parent->foot_yaw +
                                 i * foot_yaw_angle_increment);
          }
        } else {
        ...
        for (double yaw_angle : yaw_angles) {
          ...
          child->perimeter =
              compute_polygon_perimeter(polytope_surf_3d_intersect_polygon);
          ...
          while (normalized_yaw > M_PI)
            normalized_yaw -= 2.0 * M_PI;
          while (normalized_yaw < -M_PI)
            normalized_yaw += 2.0 * M_PI;
          child->foot_yaw = normalized_yaw; // Set the normalized foot yaw angle
                                            // for this child
          // Copy parent's pred_surface_ids and add parent's surface as new
          // layer
          child->pred_surface_ids = parent->pred_surface_ids;
          child->pred_surface_ids[parent->stance_foot].push_back(
              {parent->surface_id});
  ...
  auto duration_clipping =
      std::chrono::duration_cast<std::chrono::microseconds>(
          end_time_clipping - start_time_clipping);
...
void AstarSearch::plot_path() {
  ...
  if (!this->result_path.empty()) {
    std::cout << "A* Path found with " << this->result_path.size()
              << " nodes (including the root)." << std::endl;
    ...
    for (const auto &surface : this->surfaces) {
      ...
      Visualizer::add_polyhedron(renderer, surface.polyhedron_3d, surface_color,
                                 0.3);
    }
    ...
    for (const auto &node : this->result_path) {
      ...
      if (node->stance_foot == LEFT_FOOT) {
        patch_color[0] = 1.0;
        patch_color[1] = 0.0;
        patch_color[2] = 0.0; // Red
      } else {
        patch_color[0] = 0.0;
        patch_color[1] = 0.0;
        patch_color[2] = 1.0; // Blue
      }
      Visualizer::add_polyhedron(renderer, node->patch_polyhedron_3d,
                                 patch_color, 0.5);
    }
