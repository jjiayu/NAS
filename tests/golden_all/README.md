...
`NAS_SCRAMBLE=<seed>` (heap fragmentation) and `NAS_CANONICAL=<c|p|q|h|hc>` (node-key variants: `c` area centroid, `p` geometric perimeter, `q` prism perimeter on the hull, `h` convex patch) on `nas_golden_all_scenes` print a `SIG` line per scene (expansion count + path); comparing runs across seeds shows which scenes are heap-dependent. Results in `docs/paper-deltas.md`.
