...
`NAS_SCRAMBLE=<seed>` (heap fragmentation) and `NAS_CANONICAL=<letters>` (variant letters: `c` area centroid, `p` geometric perimeter, `q` prism perimeter on the hull, `h` convex patch, `t` drop near-collinear hull vertices, `s` canonical start vertex, `g` dedup by patch distance, `k` dedup by centroid+perimeter tolerance (use with `p`), `d` deterministic ties (oldest first), `l` with `d`: newest first; e.g. `htscd`); `nas_trace_divergence <scene> <seed> <letters>` reports the first divergence between two heap states and classifies it on `nas_golden_all_scenes` print a `SIG` line per scene (expansion count + path); comparing runs across seeds shows which scenes are heap-dependent. Results in `docs/paper-deltas.md`.
